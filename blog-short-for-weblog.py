...
class BaseHandler(webapp2.RequestHandler):
  ...
  def initialize(self, *a, **kw):
    ...
    uid = self.read_secure_cookie('user_id') # Makes sure user is logged in and cookie is secure
    self.user = uid and User.by_id(int(uid)) # If user logged in with cookie and user exisits, sets self.user
...
class User(db.Model):
  ...
  # Registers the user. Used to create a new user object. Does not log user in
  @classmethod
...
class EditComment(BaseHandler):
  ...
  def post (self, post_id, comment_id):
    ...
    if content:
      ...
      self.render('editcomment.html', error=error, post_id=post_id, comment_id=comment_id, content=content)
